...
Network yml2network(std::string filename) {
  ...
  std::vector<Network::Node*> current_layer;
  current_layer.resize(input_size);
  // construct the input nodes
  for (int i = 0; i < input_size; i++) {
    current_layer[i] = new Network::Node();
  }
  net.set_input_layer(current_layer);

  std::vector<Network::Node*> next_layer_activ;
  ...
  for (std::size_t i=1; i < topology.size(); i++) {
    ...
    for (int k = 0; k < next_layer_size; k++) {
      next_layer_activ[k] = new Network::Node();
    }
    ...
    for (std::size_t k=0; k < next_layer_size; k++) {
      ...
      if (activ_type.compare("Sigmoid") == 0) {
        next_layer_activ[k]->type = Network::sigmoid_act;
      }
      else if (activ_type.compare("Tanh") == 0) {
        next_layer_activ[k]->type = Network::tanh_act;
      }
      else {
